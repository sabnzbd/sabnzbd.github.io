...
3. Is the **Folder Speed** at least 150MB/s?<br>
   Both Incomplete and Complete should be on an SSD/M.2/NVMe disk.
...
5. After this, do you see **Download speed limited by** Disk Speed in the Status Window?<br>
   If so, either your disks are too slow or something is preventing SABnzbd from writing fast to your disks.
...
* Increase `Articles per request` in [Config->Servers](/wiki/configuration/{{site.wiki_version}}/servers) (Advanced settings). The default is `2`, but increasing to `4-10` can improve speed further if your server supports it. See [NNTP Pipelining](/wiki/advanced/nntp-pipelining) for more information.
...
* Enable [Direct Write](/wiki/advanced/direct-write) to reduce disk I/O when your storage is fast enough.
